...
import com.giantLink.Hiring.recrutementservice.services.Impl.UserDetailsImpl;
...
//    }
    ...
    public String generateTokenWithRole(Role role, User user) {
        ...
        return buildToken(claims, new UserDetailsImpl(user), jwtExpiration);
    }
    ...
    public String generateRefreshTokenWithRole(Role role, User user) {
        ...
        claims.put("role", role.getRoleName());
        claims.put("userId", user.getId());
        return buildToken(claims, new UserDetailsImpl(user), refreshExpiration);
    }
