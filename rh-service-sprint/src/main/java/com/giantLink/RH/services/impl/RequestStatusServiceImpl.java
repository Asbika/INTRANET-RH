...
public class RequestStatusServiceImpl implements RequestStatusService {
    ...
    public RequestStatusResponse processHolidayRequest(Long requestStatusId) {
        ...
        if (requestHoliday.isPresent()) {
            ...
            if (employeeHolidayBalance < requestedHolidays) {
                ...
                requestStatus.get().setDescription(String.format("Your holiday request has been accepted. You have sufficient holiday balance to cover the requested duration. Enjoy your well-deserved time off from %s to %s. We hope you have a wonderful vacation!", dateFormat.format(requestHoliday.get().getStartDate()), dateFormat.format(requestHoliday.get().getFinishDate())));
                ////////------- Add an ApprovedLeave based on this request info -------////////
                ApprovedLeave approvedLeave=new ApprovedLeave();
                approvedLeave.setNumberOfDays(Math.toIntExact(requestHoliday.get().getNumberOfDays()));
    ...
    public RequestStatusResponse updateStatus(RequestStatusUpdateRequest requestStatusUpdateRequest, Long requestHolidayId) {
            ...
            request.setDescription(requestStatusUpdateRequest.getMessageDetails());
            return(RequestStatusMapper.INSTANCE.entityToResponse(requestStatusRepository.save(request)));
        }
    
    @Override
    public RequestStatusResponse updateRequestStatus(Long requestId, RequestStatusRequest requestStatusRequest) {
        Request request = requestHolidayRepository.findById(requestId).get();
        ...
        requestStatus.setType(requestStatusRequest.getType());
        requestStatus.setDescription(requestStatusRequest.getMessageDetails());

        requestStatusRepository.save(requestStatus);

        return RequestStatusMapper.INSTANCE.entityToResponse(requestStatus);
    }
    }
